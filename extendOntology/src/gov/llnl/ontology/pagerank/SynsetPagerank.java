...
package gov.llnl.ontology.pagerank;
 ...
 *   that is held constant through multiple Page Rank runs, it is suggested that
 *   this core subgraph is stored in a separate list and {@link
 *   setupTransitionAttributes} be called once on this subgraph.
 *
 ...
 *   iterations.  In each iteration, the page rank score is updated and used in
 *   conjunction with a set of random surfer probabilities.
 * </ol>
 *
 * </p>
...
public class SynsetPagerank {
  ...
  /**
   * The identifier for an {@link Attribute} of transition probabilities.  These
   * {@link Attribute}s will simply be {@link SparseDoubleVector}s.
   */
   ...
   * synsetMap} provides the desired indices for each possible {@link Sysnet}.
   *
  ...
  public static void setTransitionAttribute(Synset synset,
    ...
    synset.setAttribute(
        ...
        new DoubleVectorAttribute<SparseDoubleVector>(transitionProbabilities));
  }

  /**
   * Adds transition probability {@link Attribute}s for each {@link Synset} in
   * {@code synsetList}.  The indices for outward links are specified by {@code
   * synsetMap}.
   *
   ...
   * each synset in {@code synsetList}.  Each {@link Synset} is assumed to have
   * a {@link SparseDoubleVector} {@link Attribute} representing the transition
   * probabilities, which can be setup by calling {@code
   * setupTransitionAttributes}.  Altogether, these transition vectors form the
   * transition matrix needed in the PageRank computation.   Note that if one
   * sets the {@link sourceWeights} carefully, a customized page rank can be
   * computed.  It is recomended that a small number of values in {@link
   * sourceWeights} be set to a non zero value if {@link synsetList} contains a
   * large number of {@link Synset}s from the original word net graph.
   *
   ...
   *        probability that a random surfer lands at a particular synset given
   *        that a random jump was made
   * @param weight The probability of making a random jump at any point in time
   */
  public static SparseDoubleVector computePageRank(
    ...
    for (int i = 0; i < 15; ++i) {
      ...
      // While computing the new page rank vector, compute the l1 norm of the
      // old page ranks.
      double oldL1Norm = 0;

      // Compute the initial page rank values for each vertex in the graph.
      // This is normally done by multiplying the transpose of the full
      // transition matrix by the page rank vector.  Since computing this with
      // the synset structure, we do the multiplcation in a different order.
      // The transition probabilities for each synset are multiplied by the
      // by the probability of page rank of that synset and having followed a
      // synset link.  Each probability accumulates towards the page rank of the
      // outgoing synset.
      for (int index : pageRanks.getNonZeroIndices()) {
        // Compute the probability of following a link from a synset and having
        // landed at a particular synset.
        ...
        SparseDoubleVector transitionProbs =
          (SparseDoubleVector) synset.getAttribute(TRANSITION_ATTRIBUTE).object();
        ...
        for (int rankIndex : transitionProbs.getNonZeroIndices())
          newRanks.add(rankIndex, oldScore * transitionProbs.get(rankIndex));
      }

      // Compute the difference in L1 norms for the old and new rank values.
      double newL1Norm = 0;
      ...
      // Compute the change in rank scores.  This is simply the l1 norm of the
      // difference between the two page rank vectors.  We compute this by first
      // computing the difference between indices in the old page rank scores
      // and the new page rank scores.  We then simply add in the l1 norm of all
      // the indices in the new page rank scores that were not in the old page
      // rank scores.
      double delta = 0;
