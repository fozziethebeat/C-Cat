...
/**
 * A utility class for modifying Dependency parse trees.
 *
...
public class DependencyUtil {
  ...
  /**
   * Walks through the dependency tree and adds conjunction between head words
   * and a list of words joined by a conjunction.  For example, consider the
   * sentence "There are many types of mammals, such as cats, dogs, and
   * dolphins."  If "such as" is the dominate node over the list of animals,
   * starting with "cats", and each animal node is connected to proceeding node
   * with "conj" link, then this method will add a link from "such as" to each
   * of the animal nodes.  Essentially, this bubbles up the conjunction link so
   * that elements in a list are directly connected to the head node.
   */
  public static DependencyTreeNode[] addConjunctionRelations(
    ...
    for (DependencyTreeNode treeNode : dependencyTree) {
      ...
      for (DependencyRelation link : treeNode.neighbors()) {
        // Find any nodes that are connected through a conjunction with this
        // node.  Add an artifical link between the parent of this node and
        // the "conj" child of this node.
        if (link.relation().equals("conj") &&
            link.headNode().equals(treeNode)) {
