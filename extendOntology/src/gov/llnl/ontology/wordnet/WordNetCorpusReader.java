...
public class WordNetCorpusReader implements OntologyReader {
   ...
   * particular order and have no license information.  Users should not modify
   * these files after they have been created as that will more than likely
   * corrupt offset values which are used by other WordNet interfaces.
   */
  public void saveWordNet(String newDictPath) throws IOException {
    ...
    // Precompute the size required for each relation that does not include the
    // offset size.
    ...
    // Compute the number of nodes, total number of relations stemming from each
    // node, and the total synset size for each part of speech,
      ...
      for (int pos = 0; pos < POS_TAGS.length; ++pos) {
        for (Synset synset : lemmaSynsets[pos]) {
          ...
          for (String relationType : synset.getKnownRelationTypes()) {
            int typeSize = relationType.toString().getBytes(UTF8).length + 1;
            int numRelationsPerType = synset.getRelations(relationType).size();
          ...
          for (Lemma lemma : synset.getLemmas()) {
            // Get the number of bytes need to store this lemma in utf-8 format.
          ...
          // Get the number of bytes needed to store the complete gloss.
          ...
          if (POS_TAGS[pos].equals("v")) {
            // Add 3 bytes, one for a space and two for a two byte integer.
            extraSize += 3;

            // Get the frame information.  For each frame pointer, add in the
            // bytes needed to represent the frame assignment.
          ...
          // Compute the number of bytes needed to represent this synset, minus
    ...
    // Figure out the number of bytes needed to represent the data for each part
    // of speech.  We will use the maximal number of bytes to represent every
    // offset.
    int[] offsetSize = new int[POS_TAGS.length];
    finalOffsetSize = 0;
    for (int pos = 0; pos < POS_TAGS.length; ++pos) {
      // Compute the minimum number of bytes to represent the data information
      // that does not include the offsets.
      int bytesPerOffset = Integer.toString(requiredSize[pos]).length();
      boolean converged = false;

      while (!converged) {
        // Compute the number of bytes we would use if we include the offset
        // values.
        int totalBytes = requiredSize[pos] +
                         bytesPerOffset * numNodes[pos] +
                         bytesPerOffset * numRelations[pos];
        // Compute the number of bytes needed to represent the entire data file.
        int newBytesPerOffset = Integer.toString(totalBytes).length();
        // Determine if the bytes needed to represent the full data set matches
        // the expected number of bytes needed.  If so, we are done.
        if (newBytesPerOffset == bytesPerOffset)
          converged = true;
        else
          // If we are not done, try incrementing the number of bytes per offset
          // by 1.
          bytesPerOffset++;
      }
    ...
    // Write out the data files for each part of speech.  This is done by first
    // iterating through each synset for a given part of speech, giving it an
    // offset, and then giving an offset to each of it's related synsets.  Once
    // all of it's related synsetse has an offset, they are written to their
    // respective part of speech files.
    ...
    for (int pos = 0; pos < POS_TAGS.length; ++pos) {
      if (FILE_EXTENSIONS[pos].length() != 0) {
        dataOutputStreams.add(new BufferedOutputStream(new FileOutputStream(
                newDictPath + "/data." + FILE_EXTENSIONS[pos])));
        indexOutputStreams.add(new BufferedOutputStream(new FileOutputStream(
    ...
    // lines will be written to "index.sense" in alphabetical order after all
    // sense keys have been generated.
    ...
    for (String lemmaKey : lemmaKeys) {
      ...
      for (int posIndex = 0; posIndex < POS_TAGS.length; ++posIndex) {
        for (Synset synset : lemmaSynsets[posIndex]) {
          ...
          // Initialize a queue that will hold all synsets we find starting from
          // the current synset.
          ...
          // Setup the offset size for this synset, give it an offset, and
          // compute the number of bytes needed to represent the synset.
          ...
          while (!toWriteSynsets.isEmpty()) {
            ...
            // Add the related synsets to the synsets we want to write.  We have
            // to assign offsets for each of the related synsets.
            for (Relation r : Relation.values()) {
              for (Synset relatedSynset : s.getRelations(r)) {
                ...
                // Setup the offset size and offset value, and then compute the
                // number of bytes needed to write this synset.
                related.setId(dataFileSize[relatedPos]);
                dataFileSize[relatedPos] += synsetSizes.get(related) +
                  (related.getNumRelations() + 1) * finalOffsetSize;

                // Add the synset to the set of written synsets and queue it up
                // for writing.
            ...
            dataOutputStreams.get(synsetPos).write(bytes, 0, bytes.length);
    ...
    // Next, write out the index files.  This should be relatively easy since
    // it's just a mapping from each lemma to it's possible synsets.  All of
    // this is in the lemmaPosOffsetMap.
    ...
    for (Map.Entry<String, Synset[][]> entry : lemmaPosOffsetMap.entrySet()) {
      ...
      for (int posIndex = 0; posIndex < posSynsets.length; ++posIndex) {
        ...
        // Write out the number of pointers that all senses have and then each
        // of the pointers.
        ...
        // Write out the offset for each synset matching this lemma and part of
        // speech.
        for (Synset synset : synsets)
  ...
  public void replaceSynset(Synset synset, Synset replacement) {
    if (synset.getPartOfSpeech() != replacement.getPartOfSpeech())
      throw new IllegalArgumentException(
          "Cannot replace a synset with another synset having a different " +
          "part of speech.");

    // Create the set of lemmas that the replacement synset knows about.  This
    // will be used to determine when the replacement synset is in the same
    // lemma mapping as the old synset.
    ...
    for (Lemma lemma : synset.getLemmas()) {
      ...
      // Since some lemmas, such as "G" and "g" map to the same base form in the
      // lemma map, we must skip these duplicate forms otherwise the second form
      // will try to reomve the synset from the lemma mapping a second time.
      if (seenLemmas.contains(lemmaName))
        continue;
      seenLemmas.add(lemmaName);
      ...
      // If the replacement synset is in this lemma mapping, then we have to
      // remove one of the synsets and bump the others further up.  Since
      // replacement is the one that should remain, just remove the old synset
      // from the mapping.
      if (replacementLemmas.contains(lemmaName)) {
        // Create a shortened version of the synsets matching the part of speech
        // for the synset to be removed.  Slide synsets to the right of the
        // given synset to the left by one position, and if their base lemma is
        // the current lemma, increase their sense number by 1.
        ...
        posSynsets[synset.getPartOfSpeech().ordinal()] = shortenedSynsets;
      } else {
        // Otherwise we just have to replace the old synset mapping with a
        // pointer to the new synset.
        for (int s = 0; s < synsets.length; ++s) {
          if (synsets[s] == synset)
   ...
   * readFromjar} is true, the reader will {@code dictPath} as a path within the
   * current jar running this code and read the dictionary files from the jar.
   * In these cases, {@code dictPath} should start with "/".  A common argument
   * for {@code dictPath} is "/dict", which assumes that the directory dict
   * contains all the WordNet dictionary files and is as the base directory of
   * the jar.
   */
  public static WordNetCorpusReader initialize(String dictPath,
                                               boolean readFromJar) {
  ...
  public int getMaxDepth(PartsOfSpeech pos) {
    ...
    for (Synset[][] lemmaSynsets : lemmaPosOffsetMap.values())
      for (Synset synset : lemmaSynsets[pIndex])
        maxDepths[pIndex] = Math.max(maxDepths[pIndex], synset.getMaxDepth());
    return maxDepths[pIndex];
  }
  ...
  public void saveInformationContent(Map<Synset, Integer> contentMap,
    ...
    if (finalOffsetSize == 0)
      throw new IllegalArgumentException(
          "The synset hierarchy must be saved before the information content " +
          "can be saved");
  ...
  /**
   * Sets up the verb frames, with "----" replaced with a format string so that
   * lemmas can easily be embedded into the frame via {@code String.Format}.
   */
  ...
  /**
   * Loads the lemma, part of speech to {@link Synset} map.  This also returns a
   * mapping from offset values to the relevant {@link Synset}s.
   */
  private void loadLemmaPosOffsetMap() throws IOException {
    ...
    for (String suffix : FILE_EXTENSIONS) {
      ...
      for (String line = null; (line = br.readLine()) != null; ) {
        // Skip lines that begin with a space, these are for the license.
        if (line.startsWith(" "))
          continue;

        // parse the line into individual tokens according to whitespace.
        int index = 0;
        ...
        // Get the offset values.  These correspond to the possible Synsets for
        // the given lemma.  For each offset, get the Synset from the offset to
        // synset map, if it exists, or create a new Synset for the offset.
        String[] offsets = Arrays.copyOfRange(tokens, index, tokens.length);
        ...
        // Store the synsets for this lemma in the lemma, part of speech to
        // synset map.
        ...
        if (posTag == PartsOfSpeech.ADJECTIVE)
          posToOffsets[PartsOfSpeech.ADJECTIVE_SAT.ordinal()] = synsets;
      }
  ...
  private void loadExceptionMap() throws IOException {
    ...
    // Make a copy of the exceptions for the adjective satalite parts of speech.
    posExceptionMap.set(PartsOfSpeech.ADJECTIVE_SAT.ordinal(), 
                        posExceptionMap.get(PartsOfSpeech.ADJECTIVE.ordinal())); 
  }

  /**
   * Loads the entire {@link Synset} tree into memory by parsing the data files
   * and filling in the existing {@link Synset}s that were created while loading
   * the index files.
   */
  private void loadSynsetTree() throws IOException {
    ...
    for (int pos = 0; pos < FILE_EXTENSIONS.length; pos++) {
      ...
      Map<Integer, Synset> offsetToSynsetMap = posOffsetToSynsetMap.get(pos);

      // Read each line in the data file.  Each line will contain the synset
      // offset, related lemmas, a set of relational links, a gloss, and for
      // verbs, a set of verb frames.
      BufferedReader br = getReader("data." + FILE_EXTENSIONS[pos]);
      for (String line = null; (line = br.readLine()) != null; ) {
        if (line.startsWith(" "))
          continue;

        // The data columns and gloss information are separated by a pipe.
        String[] columnAndGloss = line.split("\\|");

        // Extract the lemma terms for the synset and relational links by
        // examining the columns prior to the gloss.
        int index = 0;
        ...
        if (synset == null) {
          throw new IllegalArgumentException(
              "A synset offset was missing from the index file for line: " + 
              line + " \n, which generated offset: " + offset);
        }

        // Extract the gloss.  Glosses contain both examples and definitions.
        // Examples start with quotes and definitions do not.
        ...
        // In WordNet-2.1, some adjectives lack a definition.  In these cases,
        // simply save an empty defition, otherwise save a copy of the
        // definition that lacks the final ";" character.
        if (gloss.length() < 2)
        ...
        for (int l = 0; l < numLemmas; ++l) {
          // Lemmas sometimes have syntactic markers in the form of (pos).
          // Extract this additional marker and keep it separate from the base 
          // name.
          ...
          lemmas[l] = new BaseLemma(
              synset, lemmaName, lexName, lexNameIndex, lexId, marker);
        }

        // Extract the relations that this synset has with other synsets.  Add
        // in a relational link to the Synset graph connectring this synset to
        // other specified synsets.
        int numPointers = Integer.parseInt(columns[index++]);
        for (int p = 0; p < numPointers; ++p) {
          String symbol = columns[index++].intern();

          // Get the pointed to synset based on it's offset and part of speech.
          ...
          // Get the lemma id for this pointed to synset.  Add a related link if
          // the lemmaId is nonzero.
          String lemmaId = columns[index++];
          if (lemmaId.equals("0000"))
            synset.addRelation(symbol, pointerSynset);
          else {
            // When the lemma id is non zero it is a derivationally related
            // lemma.  Since not all lemmas have been added to their Synsets
            // yet, just add in a link based on the Synset and the lemma index
            // that corresponds to the related form.
            int sourceIndex = Integer.parseInt(lemmaId.substring(0, 2), 16);
            int targetIndex = Integer.parseInt(lemmaId.substring(2), 16);
            synset.addRelation(symbol, pointerSynset);
            synset.addDerivationallyRelatedForm(
                pointerSynset, new SimpleRelatedForm(sourceIndex, targetIndex));
          }
        }
        ...
        if (POS_TAGS[pos].equals("v")) {
          ...
          for (int f = 0; f < frameCount; ++f) {
            ...
            // Embed the specified lemmas into the verb frame and attach it to
            // each lemma object.  A lemma number of 0 signifies that all
            // lemmas should be embedded in the frame.
            ...
            if (lemmaNumber == 0) {
              ...
              for (BaseLemma lemma : lemmas)
                lemma.addFrameString(
                    String.format(frame, lemma.getLemmaName()));
            }
            else
              // Embed only the specified lemma into the frame.
              lemmas[lemmaNumber-1].addFrameString(
                  String.format(frame, lemmas[lemmaNumber-1].getLemmaName()));
          }
        ...
        for (BaseLemma lemma : lemmas) {
          lemma.setKey(String.format("%s%%%d:%02d:%02d::",
                lemma.getLemmaName(), pos+1, lemma.getLexNameIndex(),
   ...
   * may not be in Word Net.  If the given lemma has a morphological exception,
   * such as "geese", the exceptional base, e.g., "goose", form is returned
   * first.  Afterwords, a fixed set of rules are applied to the base form in
   * order.  A variant is returned for each rule that applies to the base form.
   * This {@link Iterator} ends when no more rules are applicable.
   */
  private class FormIterator implements Iterator<String> {
     ...
     * replacements available in {@code posReplacements}.  If {@code exception}
     * is non {@code null} it will be returned as the first variant.  If {@code
     * suffix} is non empty, it will be appended to each variant returned. 
     */
    ...
    /**
     * Returns the next morphological variant of the original {@code form}.  If
     * there are no possible replacements remaining, this returns {@code null}.
     */
    private String advance(String form) {
      if (replacementIndex >= posReplacements.length)
        return null;
      replacementIndex++;
      for (; replacementIndex < posReplacements.length; replacementIndex++) {
        String[] replacement = posReplacements[replacementIndex-1];
        if (form.endsWith(replacement[0])) {
          String base = form.substring(0, form.length()-replacement[0].length());
    ...
    /**
     * Returns the next morphological form of the original term.  The returned
     * form is computed by finding the next possible replacement that matches
     * the original term and changing the suffix.
     */
    public String next() {
      String curr = next;
      next = advance(form);
     ...
     */
    public void remove() {
      throw new UnsupportedOperationException("Cannot remove replacements.");
    }
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    */
    ...
    System.out.println(SynsetRelations.getCousinDistance("Attacked", "patriot", 7).toString());
    Synset first = reader.getSynset("cat", PartsOfSpeech.NOUN, 1);
    Synset second = reader.getSynset("feline", PartsOfSpeech.NOUN, 1);
    System.out.println(first.getDefinition());
    System.out.println(second.getDefinition());
    System.out.println(Similarity.path(first, second));
    System.out.println(Similarity.lch(first, second));
    System.out.println(Similarity.wup(first, second));

    InformationContent ic = new FileBasedInformationContent(args[1]);
