...
public class WordNetSynsetWriter implements SynsetWriter {
   ...
   * data.pos file format.  This method should only be called if an offset value
   * has been set for the {@link Synset} and an offset size has been set.
   */
  public String serializeSynset(Synset synset) {
    ...
    sb.append(String.format("%s ",WordNetCorpusReader.POS_TAGS[pos.ordinal()]));
    ...
    for (String relationTag : synset.getKnownRelationTypes()) {
      ...
      for (Synset r : synset.getRelations(relationTag)) {
        // Add the type of relation, offest of the related synset, and it's part
        // of speech.
        sb.append(relationTag).append(" ");
        sb.append(String.format(offsetFormat, r.getId()));
        sb.append(WordNetCorpusReader.POS_TAGS[r.getPartOfSpeech().ordinal()]);
        sb.append(" ");
        ...
        if (form == null)
          ...
          sb.append(String.format(
                "%02x%02x ", form.sourceIndex(), form.otherIndex()));
      }
    }
    ...
    if (pos == PartsOfSpeech.VERB) {
      ...
      if (frameIds == null || lemmaIds == null) {
        ...
        for (int i = 0; i < frameIds.length; ++i)
          sb.append(String.format("+ %02d %02x ", frameIds[i], lemmaIds[i]));
      }
    }
