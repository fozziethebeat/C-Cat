 ...
 * {@link Synset}
 * </li> A set of example sentences for the lemmas in the {@link Synset}
 ...
 * with {@link Relation#HYPONYM} as the relation argument.
 * </ul>
 *
 * </p>
...
public interface Synset {
  ...
  enum Relation {
    ...
    /**
     * A parent of relation.  Instance hypernyms are the reflexive type of an
     * Instance Hyponym.  For example, "river" is an Instance Hypernym of
     * "Mississippi River".
     */
    ...
    /**
     * A child of relation.  Instance hyponyms are the real world instances of a
     * generalized concept, for example, "Mississippi River" is an instance
     * hyponym of "river".
     */
     ...
     * meaning, such as "hot" and "cold".  Conceptually, they may be similar,
     * but only differ on a few distinct features.
     */
    ANTONYM("!", "!"),

    /**
     * A part of relation.  Member holonyms are parts of another concept.  For
     * example, "Saturn" is a member holonym of "solar sysetm".
     */
    MEMBER_HOLONYM("#m", "%m"),

    /**
     * A part of relation.  Substance holonyms are substances that compose
     * another object.  For example, "water" is a substancec holonym of "ice".
     */
    SUBSTANCE_HOLONYM("#s", "%s"),

    /**
     * A part of relation.  Part holonyms are concepts that are components of
     * another concept.  For example "knob" is a part holonym of "hilt".
     */
    PART_HOLONYM("#p", "%p"),
     ...
     * attribute to some other {@link Synset}.  For example, "accurate" is an
     * attribute of "truth".
     */
    ATTRIBUTE("=", null),
     ...
     * relation if it entails or implies another {@link Synset}.  For example,
     * "snore" entails "sleep".
     */
    ENTAILMENT("*", null),

    /**
     * A causal relation.  A given {@link Synset} has a cause relation if it is
     * a cause of another {@link Synset}.
     */
    CAUSE(">", null),
   ...
   * within the WordNet dictionary files.  Any {@link Synset}s that are added to
   * the dictionary during runtime may return an id of 0.  note that this value
   * is only needed for corpus readers.
   */
  ...
  /**
   * Returns the set of example sentences which explain how this {@link Synset}
   * is used in regular text.
   */
  List<String> getExamples();

  /**
   * Adds an example sentence detailing how this lemmas for this {@link Synset}
  ...
  /**
   * Returns the list of {@link Lemmas} that correspond to this {@link Synset}.
   */
  List<Lemma> getLemmas();

  /**
   * Adds a {@link Lemma} to this {@link Synset}.  {@link Lemma}s may be
   * added in order according to their precedence for the {@link Sysnet}.
   */
  ...
  /**
   * Returns the set of relation keys that this {@link Synset} has with
   * other {@link Synset}s.  In general these keys will be string versions of
   * {@link Relation}s.
   */
   ...
   * Lemma} which is a derivationally related form of a {@link Lemma} from this
   * {@link Synset}, or {@code null} if there is no relation.
   */
  RelatedForm getDerivationallyRelatedForm(Synset synset);

  /**
   * Adds a {@link RelatedForm} to this {@link Synset}, signifiying that the two
   * {@link Sysnet}s share a {@link Lemma} that are closely related.
   */
   ...
   * the IS-A hierarchy.  Since every {@link Synset} can have multiple parents,
   * there may be multiple paths that have several commond {@link Synset}s.  The
   * parent paths will begin a the {@link Synset} that is furthest from this
   * {@link Synset} and end with this {@link Synset}.
   */
  List<List<Synset>> getParentPaths();

  /**
   * Returns the {@link Synset}s that are more specific concepts of this {@link
   * Synset}.  This is equivalent to calling {@link getRelations()} with {@link
   * Relation:HYPONYM}.
   */
  ...
  /**
   * Sets {@code attribute} as an object that described this {@link Sysnet} with
   * more detail.  {@code attributeName} is a label for the provided {@code
   * attribute}.
   */
   ...
   * attributeName} applied to this {@link Sysnet}, or {@code null} if there is
