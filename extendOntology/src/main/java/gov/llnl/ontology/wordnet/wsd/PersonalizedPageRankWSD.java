...
import gov.llnl.ontology.wordnet.pagerank.SynsetPagerank;
...
public class PersonalizedPageRankWSD extends SlidingWindowDisambiguation {
    ...
    protected void processContext(Annotation focus,
        ...
        if (addTermNode(synsetList, localMap, focus) == 0)
            return;

        // Add the artificial synsets for each of the context words.
        for (Annotation prev : prevWords)
            addTermNode(synsetList, localMap, prev);
        for (Annotation next: nextWords)
            addTermNode(synsetList, localMap, next);

        // Place an even random surfer probability on each artificial synset.
        double numTerms = synsetList.size();
        SparseDoubleVector sourceWeights = new CompactSparseVector();
        for (int i = synsetMap.size(); i < localMap.size(); ++i)
            sourceWeights.set(i, 1d/numTerms);

        // Run the page rank algorithm over the created graph.
        SynsetPagerank.setupTransitionAttributes(synsetList, localMap);
        SparseDoubleVector pageRanks = SynsetPagerank.computePageRank(
                synsetList, sourceWeights, .85);

        // Determine the best sense for the focus word.  The focus word is
        // always at index 0 in the synsetList.
        Synset maxSynset = null;
        double maxValue = 0;
        for (Synset related : synsetList.get(0).getRelations("related")) {
            int index = localMap.get(related);
            if (maxValue <= pageRanks.get(index)) {
                maxValue = pageRanks.get(index);
                maxSynset = related;
            }
        }
