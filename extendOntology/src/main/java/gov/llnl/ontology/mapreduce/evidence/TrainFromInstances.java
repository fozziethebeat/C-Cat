...
public class TrainFromInstances {
  ...
  public void run(String[] args) throws Exception {
    ...
    OnlineLearner model = new AdaptiveLogisticRegression(
            attributeMap.classValues().length,
            attributeMap.numAttributes()-1,
            new L1());
    for (int i = 0; i < passes; ++i) {
      ...
      for (DataPoint d = null;
        ...
        model.train(d.value, d.dataPoint);
      br.close();
    }

    // Serialzie the trained model.
    ModelSerializer.writeJson(outputFile, model);
  }
